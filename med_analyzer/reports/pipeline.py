...
def export_report_json(report_id, extracted_data, output_dir="outputs"):
    ...
    final_report = build_final_report(report_id=report_id, extracted_data=extracted_data)
...
def generate_patient_summary(final_report):
    lines = []
    lines.append(f"Your medical report contains {final_report['total_tests']} tests.")
    if final_report["abnormal_count"] == 0:
        lines.append("All results are within normal range.")
        return " ".join(lines)
    lines.append(f"{final_report['abnormal_count']} test(s) need attention.")
    for test in final_report["abnormal_tests"]:
        lines.append(f"- {test['test_name']}: {test['value']} {test['unit']} ({test['status']}). {test['reason']}")
